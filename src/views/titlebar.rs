...
impl TitleBarView {
    ...
    fn render_settings_menu(view: PopupMenu, cx: &App) -> PopupMenu {
        let cfg = cx.global::<TideStore>().read(cx);
        let (locale, theme) = (cfg.locale(), cfg.theme());

        view
            // language menu
            .label(i18n_titlebar(cx, "language"))
            .menu_with_check("简体中文", locale == "zh-CN", Box::new(LocaleAction::ZhCN))
